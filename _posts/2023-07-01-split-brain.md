---
layout: post
title:  "How AuthNull’s Decentralized Architecture Eliminates Split Brain?"
excerpt: "AuthNull: The PAM solution that's immune to split brain. No central vault to corrupt, no single point of failure."
...
In a traditional PAM solution, split brain can be a major problem. If two or more nodes disagree about who has access to a particular system, it can lead to security breaches, data corruption, and service outages.<br/><br/>

However, AuthNull is a decentralized PAM solution that is immune to split brain. This is because AuthNull does not rely on a central server or database for password or credential storage. Instead, it uses a decentralized architecture that distributes the state of the system across a network of wallets.<br/><br/>
...
In addition to the decentralized architecture, AuthNull also uses a central database to store metadata about the wallets, such as the user's identity, their credential for  the endpoint they have access to. This ensures that even if a user loses their wallet, they can still access their credentials by logging into the central database.<br/><br/>
...
AuthNull is a decentralized PAM solution that is immune to split brain. This makes it a highly reliable and secure PAM solution that is ideal for organizations of all sizes.<br/><br/>
